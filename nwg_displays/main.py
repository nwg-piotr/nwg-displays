...
def create_workspaces_window(btn):
    ...
    btn_clear.set_label(voc["clear"])
    if sway_config_dir:
        btn_clear.connect("clicked", on_workspaces_clear_btn, dialog_win)
    else:
        btn_clear.set_sensitive(False)
    box.pack_end(btn_clear, False, False, 6)
...
def create_workspaces_window_hypr(btn):
    ...
    btn_clear.set_label(voc["clear"])
    if hypr_config_dir:
        btn_clear.connect("clicked", on_workspaces_clear_btn_hypr, dialog_win)
    else:
        btn_clear.set_sensitive(False)
    box.pack_end(btn_clear, False, False, 6)
...
def on_workspaces_clear_btn(w, win):
    global workspaces
    workspaces = {}
    save_workspaces({}, os.path.join(sway_config_dir, "workspaces"), use_desc=config["use-desc"])
    notify("Workspaces assignment", "Restart sway for changes to take effect")
    close_dialog(w, win)


def on_workspaces_clear_btn_hypr(w, win):
    global workspaces
    workspaces = {}
    workspace_conf_file = workspaces_path
    now = datetime.datetime.now()
